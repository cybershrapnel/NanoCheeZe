...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "NanoCheeZe " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"scryptseed", "seed.nanocheeze.com"},
    {"nanoseed1", "seed1.nanocheeze.com"},
    {"nanoseed2", "seed2.nanocheeze.com"},
    {"nanoseed3", "seed3.nanocheeze.com"},
    {"mequavis", "mequavis.nanocheeze.com"},
    {"shrap",   "avisc.nanocheeze.com"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("nanocoin-dnsseed");
...
{
    LogPrintf("ThreadDumpAddress started\n");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. NanoCheeZe is probably already running."), addrBind.ToString());
        else
...
void StartNode(void* parg)
{
    ...
    if (!NewThread(ThreadIRCSeed, NULL))
        LogPrintf("Error: NewThread(ThreadIRCSeed) failed\n");
    ...
    // ncz: each wallet gets its own thread.
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_SCRIPTCHECK] > 0) LogPrintf("ThreadScriptCheck still running\n");
