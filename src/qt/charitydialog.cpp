    ...
    model(0)
{
    ...
    ui->charityAddressEdit->setPlaceholderText(tr("Enter Receiving Address"));
...
void StakeForCharityDialog::setModel(WalletModel *model)
{
    ...
    if (strAddress.IsValid() && nPer > 0 )
    {
        ...
        ui->message->setText(tr("You are splitting with\n") + strAddress.ToString().c_str() + tr("."));
    }
}
...
void StakeForCharityDialog::on_enableButton_clicked()
{
    if(model->getEncryptionStatus() == WalletModel::Locked)
    {
        ...
        ui->message->setText(tr("Please unlock wallet before starting stake splitting."));
        return;
    }
...
void StakeForCharityDialog::on_disableButton_clicked()
{
    ...
    ui->message->setStyleSheet("QLabel { color: black; }");
    ui->message->setText(tr("Stake splitting is now off"));
    return;
}
