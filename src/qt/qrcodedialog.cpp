...
{
    QString ret = QString("nanocheeze:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NCZ unit input in lnReqAmount, we generate the URI with NCZ as unit (as defined in BIP21)
