...
bool DumpWallet(CWallet* pwallet, const string& strDest)
{
  ...
  while (!fShutdown)
  {
      ...
      // produce output
      file << strprintf("# Wallet dump created by NanoCheeZe %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
          ...
          if (pwallet->GetKey(keyid, key)) {
              if (pwallet->mapAddressBook.count(keyid)) {
                  ...
                  file << strprintf("%s %s label=%s # addr=%s\n",
                                    CBitcoinSecret(secret, IsCompressed).ToString(),
                                    strTime,
                                    EncodeDumpString(pwallet->mapAddressBook[keyid]),
                                    strAddr);
