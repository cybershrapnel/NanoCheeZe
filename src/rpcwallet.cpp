...
Value getinfo(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("tolbal-all-wallets",  ValueFromAmount(pWalletManager->GetTotalBalance())));
...
Value getnewaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new NanoCheeZe address for receiving payments.  "
...
Value getaccountaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current NanoCheeZe address for receiving payments to this account.");
...
Value stakeforcharity(CWallet *pWallet, const Array &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "stakeforcharity <NanoCheeZeaddress> <percent> [Change Address] [min amount] [max amount]\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NanoCheeZe address");
    ...
    if (params.size() > 2) {
        ...
        if (!changeAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NanoCheeZe change address");
        else {
            if(!IsMine(*pWallet, changeAddress.Get()))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "NanoCheeZe change address not owned");
        }
    }
...
Value setaccount(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <NanoCheeZeaddress> <account>\n"
...
Value sendtoaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <NanoCheeZeaddress> <amount> [comment] [comment-to]\n"
...
Value listaddressgroupings(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    Array jsonGroupings;
    map<CTxDestination, int64_t> balances = pWallet->GetAddressBalances();
...
Value signmessage(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <NanoCheeZeaddress> <message>\n"
...
Value verifymessage(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <NanoCheeZeaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <NanoCheeZeaddress> [minconf=1]\n"
            "Returns the total amount received by <NanoCheeZeaddress> in transactions with at least [minconf] confirmations.");
...
int64_t GetAccountBalance(CWallet* pWallet, CWalletDB& walletdb, const string& strAccount, int nMinDepth, const isminefilter& filter)
{
    int64_t nBalance = 0;
...
int64_t GetAccountBalance(CWallet* pWallet, const string& strAccount, int nMinDepth, const isminefilter& filter)
{
    CWalletDB walletdb(pWallet->strWalletFile);
...
Value getbalance(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    int64_t nBalance = GetAccountBalance(pWallet,strAccount, nMinDepth, filter);
...
Value sendfrom(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toNanoCheeZeaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase(pWallet));
    ...
    // Check funds
    int64_t nBalance = GetAccountBalance(pWallet, strAccount, nMinDepth, MINE_SPENDABLE);
    if (nAmount > nBalance)
...
Value sendmany(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (!fCreated)
    {
        ...
        if (totalAmount + nFeeRequired > nBalance - nWatchOnly)
...
static CScript _createmultisig(CWallet* pWallet, const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: NanoCheeZe address and we have full public key:
...
Value addmultisigaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
  if (fHelp || params.size() < 2 || params.size() > 3)
  {
      string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
          ...
          "each key is a NanoCheeZe address or hex-encoded public key\n"
...
void ListTransactions(CWallet* pWallet, const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret, const isminefilter& filter)
{
    ...
    if ((!wtx.IsCoinStake()) && (!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& s, listSent)
        {
            ...
            if(involvesWatchonly || (::IsMine(*pWallet, s.first) & MINE_WATCH_ONLY))
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                if(involvesWatchonly || (::IsMine(*pWallet, r.first) & MINE_WATCH_ONLY))
                   entry.push_back(Pair("involvesWatchonly", true));
...
Value gettransaction(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (pWallet->mapWallet.count(hash))
    {
        ...
        Array details;
        ListTransactions(pWallet, pWallet->mapWallet[hash], "*", 0, false, details, filter  );
...
Value walletpassphrase(CWallet* pWallet, const Array& params, bool fHelp)
{
   if (fHelp || params.size() < 2 || params.size() > 3)
    ...
    int64_t nUnlockTime = params[1].get_int64();

    if (nUnlockTime < 0 || nUnlockTime >= std::numeric_limits<int64_t>::max() / 100)
        throw runtime_error("timeout is out of bounds");

    if (nUnlockTime == 0 ) // Zero timeout means forever, well 292 epochs, close enough.
        nUnlockTime = std::numeric_limits<int64_t>::max() / 100;
...
Value encryptwallet(CWallet* pWallet, const Array& params, bool fHelp)
{
  if (fHelp || params.size() != 1)
    ...
    StartShutdown();
    return "wallet encrypted; NanoCheeZe server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <NanoCheeZeaddress>\n"
            "Return information about <NanoCheeZeaddress>.");
    ...
    if (isValid)
    {
        ...
        isminetype mine = pWallet ? IsMine(*pWallet, dest) : MINE_NO;
...
// NanoCheeZe: resend unconfirmed wallet transactions
