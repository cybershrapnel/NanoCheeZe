...
Value importprivkey(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "importprivkey <NanoCheeZeprivkey> [label]\n"
...
Value importaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
       ...
       throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NanoCheeZe address or script");
    }
...
Value dumpprivkey(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <NanoCheeZeaddress>\n"
            "Reveals the private key corresponding to <NanoCheeZeaddress>."
            + HelpRequiringPassphrase(pWallet));
