...
Value getworkex(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "NanoCheeZe is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "NanoCheeZe is downloading blocks...");
    ...
    {
	LogPrintf("WARNING1: adjustedtime %s\n", GetAdjustedTime());
	LogPrintf("WARNING1: lastblocktime %s\n", pindexBest->pprev->GetBlockTime());
	LogPrintf("WARNING1: total %s\n", GetAdjustedTime()-pindexBest->pprev->GetBlockTime());
	LogPrintf("WARNING1: blocktime %s\n", time(NULL));

        if (GetAdjustedTime() - (pindexBest->pprev->GetBlockTime()+61) < (fTestNet ? POW_TIME_LIMIT_TESTNET : POW_TIME_LIMIT))
           throw JSONRPCError(RPC_MISC_ERROR, "1:PoW must wait for PoS block or 1 min with no block");
    }
...
Value getwork(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "NanoCheeZe is downloading blocks...");
    ...
    if (pindexBest->IsProofOfWork() && pindexBest->nHeight >= (fTestNet ? POW_LIMIT_HEIGHT_TESTNET : POW_LIMIT_HEIGHT))
    {
	LogPrintf("WARNING2: adjustedtime %s\n", GetAdjustedTime());
	LogPrintf("WARNING2: lastblocktime %s\n", pindexBest->pprev->GetBlockTime());
	LogPrintf("WARNING2: total %s\n", GetAdjustedTime()-pindexBest->pprev->GetBlockTime());
	LogPrintf("WARNING2: blocktime %s\n", time(NULL));
        if (GetAdjustedTime() - (pindexBest->pprev->GetBlockTime()+61) < (fTestNet ? POW_TIME_LIMIT_TESTNET : POW_TIME_LIMIT))
           throw JSONRPCError(RPC_MISC_ERROR, "2:PoW must wait for PoS block or 1 min with no block");
    }
...
Value getblocktemplate(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (pindexBest->IsProofOfWork() && pindexBest->nHeight >= (fTestNet ? POW_LIMIT_HEIGHT_TESTNET : POW_LIMIT_HEIGHT))
    {
	LogPrintf("WARNING3: adjustedtime %s\n", GetAdjustedTime());
	LogPrintf("WARNING3: lastblocktime %s\n", pindexBest->pprev->GetBlockTime());
	LogPrintf("WARNING3: total %s\n", GetAdjustedTime()-pindexBest->pprev->GetBlockTime());
	LogPrintf("WARNING3: blocktime %s\n", time(NULL));
if (GetAdjustedTime() - (pindexBest->pprev->GetBlockTime()+61) < (fTestNet ? POW_TIME_LIMIT_TESTNET : POW_TIME_LIMIT))
           throw JSONRPCError(RPC_MISC_ERROR, "3:PoW must wait for PoS block or 1 min with no block");
    }
