...
//
// OutputDebugStringF (aka LogPrintf -- there is a #define that we really
...
#else
    const char* pszModule = "NanoCheeZe";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NanoCheeZe
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NanoCheeZe
    // Mac: ~/Library/Application Support/NanoCheeZe
    // Unix: ~/.NanoCheeZe
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NanoCheeZe";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "NanoCheeZe.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No NanoCheeZe.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override NanoCheeZe.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "NanoCheeZed.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong NanoCheeZe will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage);
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("NanoCheeZe"), CClientUIInterface::MSG_WARNING);
                }
            }
        }
        if (fDebug) {
