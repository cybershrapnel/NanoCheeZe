...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("NanoCheeZe version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  NanoCheeZed [options]                     " + "\n" +
                  "  NanoCheeZed [options] <command> [params]  " + _("Send command to -server or NanoCheeZed") + "\n" +
                  "  NanoCheeZed [options] help                " + _("List commands") + "\n" +
                  "  NanoCheeZed [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "NanoCheeZe:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("NanoCheeZe"), CClientUIInterface::MSG_ERROR);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("NanoCheeZe"), CClientUIInterface::MSG_WARNING);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n";
        ...
        strUsage += "  -conf=<file>           " + _("Specify configuration file (default: NanoCheeZe.conf)") + "\n";
        strUsage += "  -pid=<file>            " + _("Specify pid file (default: NanoCheeZed.pid)") + "\n";
        ...
        strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 12781 or testnet: 12782)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. NanoCheeZe is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  NanoCheeZe is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("NanoCheeZe version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "NanoCheeZe server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / "wallet.dat"))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("NanoCheeZe"), CClientUIInterface::MSG_WARNING);
        }
